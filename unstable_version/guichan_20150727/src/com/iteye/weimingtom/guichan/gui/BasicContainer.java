...
public abstract class BasicContainer extends Widget implements DeathListener {
	...
	public void destory() throws GuichanException {
		//FIXME:
		clear();
	}
    ...
    public void _setFocusHandler(FocusHandler focusHandler) throws GuichanException {
        ...
        while (iter.hasNext()) {
            ...
            //FIXME:
            if (w == this) {
            	 throw new GuichanException("_setFocusHandler self");
            }
            w._setFocusHandler(focusHandler);
        }
    }
	
	protected void add(Widget widget) throws GuichanException {
		//FIXME:
		if (widget == this) {
			 throw new GuichanException("Parent self");
		}
        mWidgets.addLast(widget);
        if (mInternalFocusHandler == null) {
    ...
    protected void clear() throws GuichanException {
    	Iterator<Widget> iter = mWidgets.iterator();
        while (iter.hasNext()) {
        	Widget w = iter.next();
